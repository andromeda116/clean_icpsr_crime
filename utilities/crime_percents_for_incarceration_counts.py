...
def calc_perc_fel_misd(final_df):
    """
        1.
            Get only the required crime columns for years 98-08
    """
    crime_req = final_df.loc[:, ['ORI', 'AGENCY','Govt_level', 'POP100', 'place_fips', 'placename', 'STATEFP', 'CNTY', 'YEAR',
                          'murder', 'rape', 'robbery', 'agg_assault', 'burglary', 'larceny',
                          'simple_assault', 'motor_vehicle_theft']]

    crime_years = [1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008]

    crime_req = crime_req[crime_req['YEAR'].isin(crime_years)]
    # fnl_mn_gte_10k_df_93_cr_disc_oris_df = fnl_mn_gte_10k_df_93_df[fnl_mn_gte_10k_df_93_df.ORI.isin(ini_93_cr_disc_fl.ORI)]
    ...
    """
    crime_req['tot_major_offenses_agency'] = crime_req.loc[:, ['murder', 'rape', 'robbery', 'agg_assault', 'burglary', 'larceny','simple_assault', 'motor_vehicle_theft']].sum(axis=1)
    crime_req['tot_felonies_agency'] = crime_req.loc[:, ['murder', 'rape', 'robbery','agg_assault', 'burglary', 'motor_vehicle_theft']].sum(axis=1)

    crime_req['tot_misdemeanors_agency'] = crime_req.loc[:, ['larceny', 'simple_assault']].sum(axis=1)
    ...
    crime_req.query('tot_major_offenses_agency > 0', inplace=True)

    # Sort by ORI and ascending year and get the 1st available year record for each jurisdiction
#    crime_req.sort_values(['ORI', 'YEAR'], inplace=True)

    # drop duplicates so that the 1st record, in this case the record for the first reported year is retained.
    crime_req.drop_duplicates(subset=['ORI'], inplace=True)
    crime_req.to_csv('/Users/salma/Research/clean_icpsr_crime/data/final_crime_other_main/crime_98_08_tot_non_zero.csv', index=False)


    # Getting the ORIs which don't have data for any of the years between 1998-2008
    crime_98_08_tot_zero = crime_req[~crime_req.ORI.isin(crime_req['ORI'])]
    zero_oris = set(crime_98_08_tot_zero['ORI'])

    # Write the zero ORIs to output file
    ori_list_df = pd.DataFrame(zero_oris, columns=['ORI'])
    ori_list_df.to_csv('/Users/salma/Research/clean_icpsr_crime/data/final_crime_other_main/crime_98_08_tot_zero.csv', index=False)
    ...
    crime_req_cnty_totals = crime_req.assign(
        murder_cnty_total = crime_req['murder'],
        rape_cnty_total = crime_req['rape'],
        robbery_cnty_total = crime_req['robbery'],
        agg_assault_cnty_total = crime_req['agg_assault'],
        burglary_cnty_total = crime_req['burglary'],
        larceny_cnty_total = crime_req['larceny'],
        simple_assault_cnty_total = crime_req['simple_assault'],
        motor_vehicle_theft_cnty_total = crime_req['motor_vehicle_theft']
    ).groupby(
    ...
    )  #initially reset_index(drop=True). shouldn't make a diff since we want STATEFP and CNTY to remain as columns.
    ...
    crime_req_cnty_totals['tot_felonies_cnty'] = crime_req_cnty_totals.loc[:, ['murder_cnty_total', 'rape_cnty_total', 'robbery_cnty_total',
                                                                              'agg_assault_cnty_total', 'burglary_cnty_total', 'motor_vehicle_theft_cnty_total']].sum(axis=1)

    crime_req_cnty_totals['tot_misdemeanors_cnty'] = crime_req_cnty_totals.loc[:, ['larceny_cnty_total', 'simple_assault_cnty_total']].sum(axis=1)
    crime_req_cnty_totals['tot_major_offenses_cnty'] = crime_req_cnty_totals.loc[:, ['tot_felonies_cnty', 'tot_misdemeanors_cnty']].sum(axis=1)

    crime_req_cnty_totals.to_csv('/Users/salma/Research/clean_icpsr_crime/data/final_crime_other_main/crime_totals_98_08_1st_rep_yr_cnty_totals_groupby.csv', index=False)

    # merge the county totals group by df with the crime_req on state and cnty fips so that county level totals
    # are assigned to each agency based on state and county fips
    crime_non_zero_totals = pd.merge(crime_req, crime_req_cnty_totals, on=['STATEFP', 'CNTY'])
    ...
    crime_non_zero_totals.to_csv('/Users/salma/Research/clean_icpsr_crime/data/final_crime_other_main/agency_1st_rep_yr_fel_misd_pcts_98_08.csv', index=False)


calc_perc_fel_misd(pd.read_csv('/Users/salma/Research/clean_icpsr_crime/data/final_crime_other_main/crime_icpsr_clnd_rem_main.csv'))

#
# """
#     After groupby - 3036 statefp and cnty combinations (crime_req_cnty_totals)
#
#     But in crime_req:
...
#
#     Get the list of state and cnty fips combinations which are different between crime_non_zero_totals and crime_req_cnty_totals_groupby.
# """
#
#
# """
...
#        # crime_non_zero_totals = crime_req[crime_req.Govt_level == 1].merge(crime_req_cnty_totals, on=['STATEFP', 'CNTY'], how='left')
#         crime_non_zero_totals = pd.merge(crime_req[crime_req.Govt_level == 1], crime_req_cnty_totals, on=['STATEFP', 'CNTY'], how='left')
#
#     else:
#         # Get the df with city and township level non-zero totals
#        # crime_non_zero_totals = crime_req[(crime_req.Govt_level == 2) | (crime_req.Govt_level == 3)].merge(crime_req_cnty_totals, on=['STATEFP', 'CNTY'], how='left')
#         crime_non_zero_totals = pd.merge(crime_req[(crime_req.Govt_level == 2) | (crime_req.Govt_level == 3)], crime_req_cnty_totals, on=['STATEFP', 'CNTY'], how='left')
#
